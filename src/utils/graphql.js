...
const MUTATE_UPDATE_TODO = gql`
  mutation(
    ...
    update_todoos(
      _set: { todo_done: $todo_done, todo_index: $todo_index }
      where: { user_id: { _eq: $user_id }, todo_value: { _eq: $todo_value } }
    ) {
      affected_rows
    }
  }
...
const MUTATE_DELETE_TODO = gql`
  mutation($user_id: String!, $todo_index: Int!, $todo_value: String!) {
    delete_todoos(
      where: {
        user_id: { _eq: $user_id }
        todo_value: { _eq: $todo_value }
